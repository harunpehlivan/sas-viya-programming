...
def plot_imgs(cas_table, class_list=range(10), images_per_class=2, figsize=(20,20), query_condition=None, font_size=12):
    ...
    for i in class_list:
        a = (
            cas_table.groupby(['_label_'])
            .get_group([f'class{str(i)}'])
    ...
    for x,ax in enumerate(axes.flat):
        ...
        ax.set_title(f'True label: {class_description[lbl_df.iloc[x]]}')
        ax.title.set_fontsize(font_size)
        ax.xaxis.label.set_fontsize(font_size)

        if query_condition != None:
            ax.set_xlabel(f'Pred label: {class_description[prd_df.iloc[x]]}')
